...
# load model
n_perform_fit3 <- readRDS(here::here("gaze_entropy_heterogenous/models/n_back_performance_model_3.rds"))
...
# load model
sge.fit.3 <- readRDS(here::here("gaze_entropy_heterogenous/models/sge_fit_3.rds"))
...
*add_epred_draws()* adds draws from the expectation of the posterior predictive distribution. Provides the expected means from the model. The default for the re_formula is NULL which includes the groups level effects which thus produces marginal effects (incorportates the random effects in the estimate). When this is plotted along with intervals, it refers to the posterior mean alongside 50% and 95% quantile credible intervals.  
...
In the code chunk above (and the same for the gaze transitions entropy model), I am calculating the conditional effects for the existing groups in the dataset (AKA what is the effect of N-back on gaze entropy for each individual). 

I do this by extracting the random effects parameter estimates directly from the model, and adding the conditional mode to the average effect. In other tutorials (https://www.andrewheiss.com/blog/2021/11/10/ame-bayes-re-guide/#conditional-effects-for-existing-groups) they use a different method with the emmeans functions. Overall, the mean value of the conditional effect is identical. However in the Heiss tutorial, each estimate is accompanied by a 95% HDIs for each individual. This might prove useful for further analysis if needed, but it not used in the current manuscript. 

To illustrate the equivalence, I'll use the Heiss method below. When viewed with the View() function, the average for each participant is the same as the method above. 
...
# loading gte model
gte.fit.6 <- readRDS(here::here("gaze_entropy_heterogenous/models/gte_fit_6.rds"))
...
#
ppid.effects <- cbind(ppid = rownames(ppid.effects), ppid.effects)

ppid.effects <- merge(ppid.effects, participant.info, by = c("ppid"))

ggplot(ppid.effects, mapping = aes(age, ranef)) +
  geom_point() +
