...
```{r}
# Setting digits to 15 mins that fixation duration s are not rounded to annoying precise values. 
options(digits = 15)
...
maxY = max(example.fixation$pitch)
maxX = max(example.fixation$yaw)

# minimum pitch and yaw coordinates
minY = min(example.fixation$pitch)
minX = min(example.fixation$yaw)

# creating state spaces - cuts the x and y coordinates into 3 degree chunks 
example.fixation <- example.fixation %>%
...
state_space_coordinates <- example.fixation %>%
...
fixation_state_space <- ggplot() +
  ...
  geom_point(example.fixation, mapping = aes(yaw, pitch, col = fixation_number), size = .6) +
...
# data saving   
ggsave(here::here("gaze_entropy_heterogenous/plots/Fig4.tiff"), plot = fixation_state_space , width = 12, height = 8, units = 'cm', dpi = 300, type = 'cairo')
```
