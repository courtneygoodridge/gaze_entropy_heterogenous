...
This is the theme plot used all of the plots. Of course, each plot also has it's own idiosyncrasies but this is a good baseline. 
...
This data contains every fixation for each participant for the automated driving periods. This is really only used for Figure 4 where the state spaces and fixations are described.

```{r}
# Setting digits to 15 mins that fixation duration s are not rounded to annoying precise. 
options(digits = 15)
fixations <- fread(file = here::here("ITS/Work package 6.5 (HiDrive)/TME study/Data/Eye data/fixations.csv"))
```
...
```{r}
entropy.total <- fread(file = here::here("ITS/Work package 6.5 (HiDrive)/TME study/Data/Eye data/gaze entropy/Critical and Non-Critical/entropy.total.csv"))
```
...
participant.info <- read.csv(here::here("ITS/Work package 6.5 (HiDrive)/TME study/Data/participant_info.csv")) %>%
  dplyr::select(1:4, 7) %>%
...
```{r}
nasa.tlx.mental <- fread(file = here::here("ITS/Work package 6.5 (HiDrive)/TME study/Data/NASA_TLX_scores.csv")) %>%
...
# fit 4 - random intercepts with priors on intercept (no n-back) and slope parameter (subjective workload)
...
## Manuscript Figure 4 - fixations within their state spaces

An example of the fixations for a trial. This is to demonstrate the theory beyond gaze transition entropy. The order of the fixations are coloured to indicate their order. The probability of a fixation being in a state space given is it in the current one is used alongside the probability of the total distribution of fixations.

```{r}
# maximum pitch and yaw coordinates
maxY = max(fixations$pitch)
maxX = max(fixations$yaw)

# minimum pitch and yaw coordinates
minY = min(fixations$pitch)
minX = min(fixations$yaw)

# example trial - this can be any trial
example <- fixations %>%
  dplyr::filter(trialid == "9_FALSE_6_3")

# creating state spaces - cuts the x and y coordinates into 3 degree chunks 
example <- example %>%
  dplyr::mutate(x_range = cut(yaw, seq(minX, maxX, by = 3), right = FALSE)) %>%
  dplyr::mutate(y_range = cut(pitch, seq(minX, maxY, by = 3), right = FALSE))

# state space coordinates
state_space_coordinates <- example %>%
  dplyr::group_by(x_range, y_range) %>%
  dplyr::summarise(count = n()) %>%
  dplyr::ungroup()  %>%
  tidyr::complete(x_range, y_range) %>%
  dplyr::mutate(state_space = row_number()) %>%
  dplyr::mutate(x_range_2 = x_range, y_range_2 = y_range) %>%
  tidyr::separate_wider_delim(x_range, ",", names = c("x_lower", "x_upper")) %>%
  tidyr::separate_wider_delim(y_range, ",", names = c("y_lower", "y_upper")) %>%
  dplyr::mutate(x_lower = str_replace_all(x_lower, "\\[|\\]", ""), x_upper = str_replace_all(x_upper, "\\(|\\)", "")) %>%
  dplyr::mutate(y_lower = str_replace_all(y_lower, "\\[|\\]", ""), y_upper = str_replace_all(y_upper, "\\(|\\)", "")) %>%
  dplyr::mutate(x_lower = as.numeric(x_lower), x_upper = as.numeric(x_upper), y_lower = as.numeric(x_lower), y_upper = as.numeric(x_upper))

# plot the state space grids alongside the fixation coordinates 
fixation_state_space <- ggplot() +
  geom_vline(state_space_coordinates, mapping = aes(xintercept = x_upper), col = "grey") +
  geom_hline(state_space_coordinates, mapping = aes(yintercept = y_upper), col = "grey") +
  geom_vline(state_space_coordinates, mapping = aes(xintercept = x_lower), col = "grey") +
  geom_hline(state_space_coordinates, mapping = aes(yintercept = y_lower), col = "grey") +
  geom_point(example, mapping = aes(yaw, pitch, col = fixation_number), size = .6) +
  scale_colour_continuous(name = "Fixation number", type = "viridis", breaks = c(25, 75, 125)) +
  ylab("Pitch (°)") +
  xlab("Yaw (°)") +
  ylim(-20, 10) +
  xlim(-40, 50) +
  theme_plot +
  theme(legend.position = "bottom", legend.key.width = unit(1, 'cm'), legend.key.height = unit(.3, 'cm'))
  
# data saving   
ggsave(here::here("ITS/Work package 6.5 (HiDrive)/TME study/Analysis/Plots/manuscript plots/fixation_state_space .tiff"), plot = fixation_state_space , width = 12, height = 8, units = 'cm', dpi = 600, type = 'cairo')
```

## Manuscript Figure 5: Correlation between N-back performance and age
...
```{r}
# load n-back data
n_back_performance <- fread(file = here::here("ITS/Work package 6.5 (HiDrive)/TME study/Data/Vehicle data/Processed data/n_back/n_back_performance.csv"))
...
n_back_age_performance <- n_back_performance %>%
  ...
  xlim(-2, 2.5) +
  theme_plot +
  theme(legend.position = c(.53, .95), legend.direction = "horizontal", legend.key = element_blank(), legend.key.width = unit(0.3, 'cm'), legend.key.size = unit(0.1, 'cm'), plot.title = element_text(size = 7.5, face = "bold"), axis.title.x = element_text(size = 10), axis.title.y = element_text(size = 10), axis.text.y = element_text(size = 8), axis.text.x = element_text(size = 10), title = element_text(size = 7), legend.title = element_text(size = 11), legend.text = element_text(size = 10)) 

# plot saving
ggsave(here::here("ITS/Work package 6.5 (HiDrive)/TME study/Analysis/Plots/manuscript plots/n_back_age_performance.tiff"), plot = n_back_age_performance , width = 12, height = 8, units = 'cm', dpi = 600, type = 'cairo')
```
...
## Manuscrip Figure 6 - Posterior distribution for mu parameters
...
beta_l <- sge.fit.3 %>%
  ...
  scale_x_continuous(limits = c(-.05, 0), breaks = c(-.04, -.02, 0)) +
  theme_plot +
  theme(legend.position = "bottom", legend.direction = "horizontal", legend.key = element_blank(), legend.key.width = unit(0.3, 'cm'), axis.text.y = element_blank(), axis.ticks.y = element_blank(),axis.ticks = element_blank(), legend.key.size = unit(0.1, 'cm'), plot.title = element_text(size = 7.5, face = "bold"), axis.title.x = element_text(size = 11), axis.text.x = element_text(size = 11), title = element_text(size = 7), legend.title = element_text(size = 11), legend.text = element_text(size = 10)) 
...
# combinding plots into one big plot 
sge.plot <- (beta_0 | beta_n) / (beta_l | beta_nl)

# saving final plot
ggsave(here::here("ITS/Work package 6.5 (HiDrive)/TME study/Analysis/Plots/manuscript plots/posterior.mu.tiff"), plot = sge.plot, width = 12, height = 8, units = 'cm', dpi = 600, type = 'cairo')
```

## Manuscript Figure 7 - Posterior distribution for sigma parameters
...
alpha_0 <- sge.fit.3 %>%
  ...
  stat_halfeye(.width = c(.5, .95)) +
  ylab("") +
  scale_x_continuous(limits = c(-3.4, -2.7), labels = label_number(accuracy = 0.01)) +
...
# saving plots
ggsave(here::here("ITS/Work package 6.5 (HiDrive)/TME study/Analysis/Plots/manuscript plots/posterior.sigma.tiff"), plot = sge.plot.sigma, width = 12, height = 8, units = 'cm', dpi = 600, type = 'cairo')
```

## Manuscript Figure 8: overlying raw data on posterior predicitive intervals
...
*add_epred_draws()* adds draws from the expectation of the posterior predictive distribution. Provides the expected means from the model.
...
posterior.mean.intervals.sge <- entropy.total %>%
  ...
  ylab("Secondary task") +
  scale_x_continuous(breaks = seq(0, .6, .1), labels = label_number(accuracy = 0.01)) +
  facet_wrap(~ lead, labeller = labeller(lead = l.labs),  ncol = 1) +
  theme_plot +
  theme(legend.position = c(.5, .95), legend.direction = "horizontal", legend.key = element_blank(), legend.key.width = unit(0.3, 'cm'), legend.key.size = unit(0.1, 'cm'), plot.title = element_text(size = 7.5, face = "bold"), axis.title.x = element_text(size = 11), axis.text.x = element_text(size = 11), title = element_text(size = 7), legend.title = element_text(size = 11), legend.text = element_text(size = 10)) 

# plot saving
ggsave(here::here("ITS/Work package 6.5 (HiDrive)/TME study/Analysis/Plots/manuscript plots/posterior.mean.intervals.sge.n.tiff"), plot = posterior.mean.intervals.sge, width = 16, height = 10, units = 'cm', dpi = 600, type = 'cairo')
```

## Manuscript Figure 9 part 1: Calculating heterogenity of the n-back effect for SGE 
...
HI_effect_2_back_plot <- ggplot() +
  ...
  geom_hline(aes(yintercept = summary(sge.fit.3)[["fixed"]][3,1] - 1.96 * ran.effects, linetype = "95% HI"), size = 1.5, color = viridis(5)[2]) +
  scale_linetype_manual(name = " ", values = c(2, 1, 1), labels = c("95% CI", "95% HI", expression(beta[N])), guide = guide_legend(override.aes = list(color = c(viridis(5)[3], viridis(5)[2], "black")))) +
  scale_fill_manual(name = "Parameter", values = c(viridis(5)[2])) +
  theme(legend.position = c(.22, .85), legend.key = element_blank(), legend.key.width = unit(0.2, 'cm'), axis.text.y = element_blank(), axis.ticks.y = element_blank(), legend.key.size = unit(0.1, 'cm'), plot.title = element_text(size = 7.5, face = "bold"), axis.title.x = element_text(size = 11), axis.text.x = element_text(size = 11), title = element_text(size = 7), legend.title = element_text(size = 11), legend.text = element_text(size = 10)) +
  coord_flip()  
...
## Manuscript 9 part 2: Proportion of population who has reduced SGE during n-back
...
proportion_pop_2_back <- densplot + 
  ...
  ylab("") +
  scale_x_continuous(limits = c(-.4, .25), expand = c(0, 0), labels = label_number(accuracy = 0.01)) +
...
# data saving
ggsave(here::here("ITS/Work package 6.5 (HiDrive)/TME study/Analysis/Plots/manuscript plots/proportion_pop_2_back.tiff"), plot = population.plot, width = 16, height = 8, units = 'cm', dpi = 600, type = 'cairo')
```
...
## Manuscript Figure 10 - Visualing posterior distribution for mu parameters - GTE
...
beta_n <- gte.fit.6 %>%
  ...
  ylab("") +
  xlab(expression(italic(p(beta[N] ~ "|" ~ italic(E))))) +
  scale_x_continuous(limits = c(-.045, .02), breaks = c(-.04, -.02, 0, .02),labels = label_number(accuracy = 0.01)) +
  theme_plot +
  theme(legend.position = "bottom", legend.direction = "horizontal", legend.key = element_blank(), legend.key.width = unit(0.3, 'cm'), axis.text.y = element_blank(), axis.ticks.y = element_blank(),axis.ticks = element_blank(), legend.key.size = unit(0.1, 'cm'), plot.title = element_text(size = 7.5, face = "bold"), axis.title.x = element_text(size = 11), axis.text.x = element_text(size = 11), title = element_text(size = 7), legend.title = element_text(size = 11), legend.text = element_text(size = 10)) 
...
beta_l <- gte.fit.6 %>%
  ...
  ylab("") +
  xlab(expression(italic(p(beta[L] ~ "|" ~ italic(E))))) +
  scale_x_continuous(limits = c(-.004, .006), breaks = c(-.003, 0, .003)) +
  theme_plot +
  theme(legend.position = "bottom", legend.direction = "horizontal", legend.key = element_blank(), legend.key.width = unit(0.3, 'cm'), axis.text.y = element_blank(), axis.ticks.y = element_blank(),axis.ticks = element_blank(), legend.key.size = unit(0.1, 'cm'), plot.title = element_text(size = 7.5, face = "bold"), axis.title.x = element_text(size = 11), axis.text.x = element_text(size = 11), title = element_text(size = 7), legend.title = element_text(size = 11), legend.text = element_text(size = 10)) 
...
ggsave(here::here("ITS/Work package 6.5 (HiDrive)/TME study/Analysis/Plots/manuscript plots/posterior.mu.gte.tiff"), plot = gte.plot, width = 12, height = 8, units = 'cm', dpi = 600, type = 'cairo')
```

## Manuscript Figure 11 - Visualing posterior distribution for sigma parameters - GTE
...
alpha_0 <- gte.fit.6 %>%
  ...
  stat_halfeye(.width = c(.5, .95)) +
  ylab("") +
  scale_x_continuous(limits = c(-5.1, -4), breaks = c(-5, -4.7, -4.4, -4.1),labels = label_number(accuracy = 0.01)) +
  xlab(expression(italic(p(alpha[0] ~ "|" ~ italic(E))))) +
  theme_plot +
  theme(legend.position = "bottom", legend.direction = "horizontal", legend.key = element_blank(), legend.key.width = unit(0.3, 'cm'), axis.text.y = element_blank(), axis.ticks.y = element_blank(),axis.ticks = element_blank(), legend.key.size = unit(0.1, 'cm'), plot.title = element_text(size = 7.5, face = "bold"), axis.title.x = element_text(size = 11), axis.text.x = element_text(size = 11), title = element_text(size = 7), legend.title = element_text(size = 11), legend.text = element_text(size = 10)) 
...
ggsave(here::here("ITS/Work package 6.5 (HiDrive)/TME study/Analysis/Plots/manuscript plots/posterior.sigma.gte.tiff"), plot = sge.plot.sigma, width = 12, height = 8, units = 'cm', dpi = 600, type = 'cairo')
```

## Manuscript Figure 12: overlying raw data on posterior predicitive intervals for gaze transition entropy model
...
ggsave(here::here("ITS/Work package 6.5 (HiDrive)/TME study/Analysis/Plots/manuscript plots/posterior.mean.intervals.gte.n.tiff"), plot = posterior.mean.intervals.gte, width = 16, height = 10, units = 'cm', dpi = 600, type = 'cairo')
```

## Manuscript Figure 13 part 1: Calculating heterogenity of the n-back effect for GTE
...
HI_effect_2_back_plot_mu <- ggplot() +
  ...
  xlab("") + 
  scale_y_continuous(limits = c(-.09, .06), breaks = c(-.05, 0, .05), labels = label_number(accuracy = 0.01)) +
  geom_hline(aes(yintercept = bayes.credible.intervals$`l-95% CI`[3], linetype ="95% CI"), size = 1.5,  color = viridis(5)[3]) +
  geom_hline(aes(yintercept = bayes.credible.intervals$`u-95% CI`[3], linetype="95% CI"), size = 1.5,  color = viridis(5)[3]) +
  ...
  theme(legend.position = c(.27, .85), legend.key = element_blank(), legend.key.width = unit(0.2, 'cm'), axis.text.y = element_blank(), axis.ticks.y = element_blank(), legend.key.size = unit(0.1, 'cm'), plot.title = element_text(size = 7.5, face = "bold"), axis.title.x = element_text(size = 11), axis.text.x = element_text(size = 11), title = element_text(size = 7), legend.title = element_text(size = 11), legend.text = element_text(size = 10)) +
  coord_flip()  
...
## Manuscript Figure 13 part 2: proportion of population who follow average GTE
...
proportion_pop_2_back_gte <- densplot + 
  ...
  ylab("") +
  scale_x_continuous(limits = c(-.16, .15), expand = c(0, 0)) +
...
ggsave(here::here("ITS/Work package 6.5 (HiDrive)/TME study/Analysis/Plots/manuscript plots/proportion_pop_2_back_gte.tiff"), plot = population.plot.gte, width = 16, height = 8, units = 'cm', dpi = 600, type = 'cairo')
```

## Manuscript Figure 14 part 1: Heterogenity effects of GTE on sigma parameters
...
HI_effect_2_back_plot_sigma <- ggplot() +
  ...
  geom_hline(aes(yintercept = summary(gte.fit.6)[["fixed"]][6,1] - 1.96 * ran.effects, linetype = "95% HI"), size = 1.5, color = viridis(5)[2]) +
    scale_linetype_manual(name = " ", values = c(2, 1, 1), labels = c("95% CI", "95% HI", expression(alpha[N])), guide = guide_legend(override.aes = list(color = c(viridis(5)[3], viridis(5)[2], "black")))) +
  scale_fill_manual(name = "Parameter", values = c("dodgerblue2")) +
  theme(legend.position = c(.204, .85), legend.key = element_blank(), legend.key.width = unit(0.2, 'cm'), axis.text.y = element_blank(), axis.ticks.y = element_blank(), legend.key.size = unit(0.1, 'cm'), plot.title = element_text(size = 7.5, face = "bold"), axis.title.x = element_text(size = 11), axis.text.x = element_text(size = 11), title = element_text(size = 7), legend.title = element_text(size = 11), legend.text = element_text(size = 10)) +
  coord_flip()  
...
## Manuscript Figure 14 part 2: Proportion of population where sigma of GTE decreases in the presence of n-back
...
ggsave(here::here("ITS/Work package 6.5 (HiDrive)/TME study/Analysis/Plots/manuscript plots/proportion_pop_2_back_gte_sigma.tiff"), plot = population.plot.sigma.gte, width = 16, height = 8, units = 'cm', dpi = 600, type = 'cairo')
```

## Manuscript figure 15 Stationary gaze entropy - influence of age on heteroenity effects
...
Here we investigate whether age can predict the heterogeneity in the effect that N-back has on gaze entropy. We include age into the model as an interaction term. Total variance in gaze entropy attributed to n-back is compute, and then we work out how much of this is explained by age alone. 
...
age.nback.effect.sge <- ggplot(ageranef, aes(age.c, ranef.age.pred.tot)) + 
  ...
  ylab(expression("Individual effects of N-back on"~ H[s])) +
  #ylim(-.15, .1) +
  theme_plot +
    theme(legend.position = c(.5, .95), legend.direction = "horizontal", legend.key = element_blank(), legend.key.width = unit(0.3, 'cm'), legend.key.size = unit(0.1, 'cm'), plot.title = element_text(size = 7.5, face = "bold"), axis.title.x = element_text(size = 11), axis.text.x = element_text(size = 11), axis.title.y = element_text(size = 11), axis.text.y = element_text(size = 11), title = element_text(size = 7), legend.title = element_text(size = 11), legend.text = element_text(size = 10)) 

ggsave(here::here("ITS/Work package 6.5 (HiDrive)/TME study/Analysis/Plots/manuscript plots/fixation_state_space .tiff"), plot = age.nback.effect.sge , width = 12, height = 8, units = 'cm', dpi = 600, type = 'cairo')
```

## Manuscript figure 16 gaze transition entropy - influence of age on heteroenity effects

Same principle as above but this time for gaze transition entropy. 19% of the heterogeneity in the effect of N-back is explained by age. Younger people tend to show increases in gaze transition entropy as a function of n-back (opposite of the average causal effect) or show null effects. Older people tend have reductions in gaze transition entropy randomness of fixations when having high mental workload.  
...
age.nback.effect.gte <- ggplot(ageranef, aes(age.c, ranef.age.pred.tot)) + 
  ...
  geom_jitter(height = 0, width = 0, size = 4,
              shape = 21, colour = "black", fill = viridis(5)[3], alpha = .95, stroke = 1) + 
  xlab("Age (mean centered)") +
  ylab(expression("Individual effects of N-back on"~ H[t])) +
  #ylim(-.15, .1) +
  theme_plot +
    theme(legend.position = c(.5, .95), legend.direction = "horizontal", legend.key = element_blank(), legend.key.width = unit(0.3, 'cm'), legend.key.size = unit(0.1, 'cm'), plot.title = element_text(size = 7.5, face = "bold"), axis.title.x = element_text(size = 11), axis.text.x = element_text(size = 11), axis.title.y = element_text(size = 11), axis.text.y = element_text(size = 11), title = element_text(size = 7), legend.title = element_text(size = 11), legend.text = element_text(size = 10)) 
 
ggsave(here::here("ITS/Work package 6.5 (HiDrive)/TME study/Analysis/Plots/manuscript plots/age.nback.effect.tiff"), plot = age.nback.effect.gte, width = 12, height = 8, units = 'cm', dpi = 600, type = 'cairo')
```
